...
Packs `dir` (which must contain a `patchcord-app.yaml` manifest) into a `.patchcord-app` archive that `app install` can install directly. `-o/--output` defaults to `<id>-<version>.patchcord-app` in the current directory.
