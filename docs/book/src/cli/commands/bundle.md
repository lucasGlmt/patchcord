...
Installs straight from `dir` — no `bundle pack`/`bundle install` round trip, no checksum, no signature (`dir` is local, unsigned-by-design source under active development). The embedded app (if any) is installed in place exactly as [`app dev`](app.md) would: served live from `dir`'s app subdirectory, never copied or moved, so rebuilding it (e.g. `vite build --watch`) needs no further agent involvement.

Each embedded workflow is installed under a dev-only rule ([ADR-0055](../../../../adr/0055-auto-incrementation-de-version-des-workflows-embarques-en-mode-developpement.md)): byte-identical content at its declared `version` is a no-op, and content that *differs* from an already-used version — edited the workflow's body, forgot to bump `version` — is installed under the next unused version automatically, instead of being rejected. The file on disk is never rewritten; only the internally-recorded copy's declared version is normalized to match. `bundle install`/`bundle update` are unaffected: they still enforce immutable, developer-bumped versions ([ADR-0008](../../../../adr/0008-workflows-publies-immuables.md)) with no dev-mode exception — that path installs a package a developer chose to publish. `requires_plugins` is still enforced — a missing dependency is not installed automatically.

`--watch` keeps the command running, reinstalling on every change under `dir` (debounced ~300ms to coalesce a build tool's burst of writes into one reinstall) until interrupted with Ctrl+C. An install failure while watching (e.g. a missing required plugin, or invalid YAML) is printed but does not stop the watch — fix it and save again. Hidden directories (`.git` and similar) are never watched.

See also [`patchcord dev`](../dev.md), which runs the agent, watches a bundle directory, and starts the embedded app's own dev server — the three commands this page and [Serving the Agent](../serve.md) otherwise require in separate terminals.
