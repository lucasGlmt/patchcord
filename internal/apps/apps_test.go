...
func TestPackAndInstallPackage(t *testing.T) {
	...
	a, _, err := InstallPackage(context.Background(), db, dataDir, packagePath, false)
	if err != nil {
	...
	t.Run("duplicate id is rejected like a directory install", func(t *testing.T) {
		if _, _, err := InstallPackage(context.Background(), db, dataDir, packagePath, false); !errors.Is(err, ErrAlreadyExists) {
...
func TestInstallPackage_SignatureAndTrustPolicy(t *testing.T) {
	sourceDir := newTestAppDir(t, "signed-dashboard", "0.1.0")
	pub, priv, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatalf("GenerateKey() error = %v", err)
	}

	packagePath := filepath.Join(t.TempDir(), "signed-dashboard.patchcord-app")
	f, err := os.Create(packagePath)
	if err != nil {
		t.Fatalf("create package file: %v", err)
	}
	...
	t.Run("signed but untrusted key: proceeds without requireSignature", func(t *testing.T) {
		db := openTestDB(t)
		a, policy, err := InstallPackage(context.Background(), db, t.TempDir(), packagePath, false)
		if err != nil {
			t.Fatalf("InstallPackage() error = %v", err)
		}
		if a.ID != "signed-dashboard" {
			t.Fatalf("ID = %q, want %q", a.ID, "signed-dashboard")
		}
	...
	t.Run("signed and trusted: accepted with requireSignature", func(t *testing.T) {
		db := openTestDB(t)
		if err := trust.Add(context.Background(), db, "signed-dashboard", pub, "test"); err != nil {
			t.Fatalf("trust.Add() error = %v", err)
		}

		a, policy, err := InstallPackage(context.Background(), db, t.TempDir(), packagePath, true)
		if err != nil {
			t.Fatalf("InstallPackage() error = %v", err)
		}
		if a.ID != "signed-dashboard" {
	...
	t.Run("unsigned package rejected with requireSignature", func(t *testing.T) {
		unsignedDir := newTestAppDir(t, "unsigned-dashboard", "0.1.0")
		unsignedPath := filepath.Join(t.TempDir(), "unsigned-dashboard.patchcord-app")
